...
demo = {
    ...
    initLBDWizard: function(form) {
        ...
        function eventDateCheck(){
            if($(`${form} [name="eventEndDateTime"]`).val()<$(`${form} [name="eventStartDateTime"]`).val())
            {
                demo.showNotification('top','right','End Date Must Be Greater Than or Equal To Start Date','danger','')
                return false;
            }
            return true;
        }
        function eventTimeCheck(){
            let startTime=parseFloat(($(`${form} [name="startDateTime"]`).val()).replace(":","."));
            let endTime=parseFloat(($(`${form} [name="endDateTime"]`).val()).replace(":","."));
            let diff=(endTime<=startTime) ?
            (24-(startTime - endTime)) : (endTime - startTime);

            if(diff > 12)
            {
                demo.showNotification('top','right','Start And End Time Must Not Exceed 12 Hours','danger','')
                return false;   
            }
            return true;
        }
        function eventCheck(){
            if($(`${form} [name="eventStartDateTime"]`).val() === $(`${form} [name="eventEndDateTime"]`).val())
            {
                if(new Date(`${$(`${form} [name="eventStartDateTime"]`).val()} ${$(`${form} [name="startDateTime"]`).val()}`) <= new Date())
                {
                    demo.showNotification('top','right','Event Time Has Passed','danger','')
                    return false;
                }
            }
            return true;
        ...
        $(`${form} .card-wizard`).bootstrapWizard({
            'tabClass': 'nav nav-pills',
            'nextSelector': '.btn-next',
            'previousSelector': '.btn-back',

            onNext: function(tab, navigation, index) {
                //console.log("on Next",tab, navigation, index);
                ...
                if(!eventDateCheck()){return false};
                if(!eventTimeCheck()){return false};
                if(!eventCheck()){return false};
            },
            ...
            onTabShow: function(tab, navigation, index) {
                ...
                var $wizard = navigation.closest(`${form} .card-wizard`);
    ...
    showNotification: function(from, align,message,colors,icon) {
        color = Math.floor((Math.random() * 4) + 1);

        $.notify({
            icon: icon || "nc-icon nc-app",
            message: message
